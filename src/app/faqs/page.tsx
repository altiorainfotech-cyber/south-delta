  ...
  return (
    <>
      ...
      <section className="relative w-full min-h-screen overflow-hidden">
        ...
        {/* Content */}
        <div
          className={`relative z-10 flex flex-col h-full px-8 md:px-16 lg:px-24 pt-[180px] pb-16 transition-all duration-[1500ms] ease-out ${
        ...
        >
          ...
          <div className="max-w-2xl mb-8">
            {/* Main Title */}
            <h1 className="font-rowdies text-[60px] font-regular leading-tight tracking-tight text-white mb-6">
              FREQUENTLY ASKING
              <br />
              ANY QUESTIONS
            </h1>
            ...
            {/* Description */}
            <p className="font-inter text-[18px] text-white leading-relaxed max-w-xl mb-8">
          ...
          {/* FAQ Accordion */}
          <div
            className={`max-w-xl bg-[#043E93] rounded-lg p-6 transition-all duration-[1500ms] ease-out delay-500 ${
              isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
          >
            {faqData.map((faq) => (
              <div key={faq.id} className="border-b border-white/10 last:border-b-0">
                ...
                >
                  <span className="font-inter text-[16px] font-medium text-[#FDC052]">
                    {faq.id}. {faq.question}
                  </span>
                  <span className="text-[#FDC052] text-2xl ml-4">
                    {openFaq === faq.id ? "−" : "+"}
                  </span>
                </button>
                {openFaq === faq.id && (
                  <div className="pb-4">
                    <p className="font-inter text-[14px] text-white/80 leading-relaxed pl-4">
                      {faq.answer}
                    </p>
                  </div>
                )}
